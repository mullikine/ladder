...
/* this function will update the stats affected by the match
   recursively acting upon parent ladders
 */
function update_stats(&$players, $teamwon, $ladder) {
  echo '<h3>Updating ratings</h3><ul>';
  ...
  for ($i = 0; $i < count($players); $i++) {
    ...
    if ($result == 0) {
      ...
      $query = 'INSERT INTO stats (ladder, gr_id, rank, pointswon) VALUES(\''.$ladder.'\',\''.$players[$i][0].'\',\'1600\', \'0\');';
      $result = mysql_query($query) or die(mysql_error());
...
\''.$players[$i][0].'\',\'1600\', \'0\');';
  ...
  $diffrating = abs($teamrating[2 - $teamwon] - $teamrating[$teamwon - 1]);
  ...
  if ($unisonladder) {
    echo '<h3>Updating parent ladder '.$unisonladder.'</h3>';
    update_stats($players,$teamwon,$unisonladder);
  }
